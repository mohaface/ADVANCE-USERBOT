...
from Rocky.database.pmpermitdb import get_approved_users, pm_guard
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Rocky.set_block_message(Rocky.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Rocky.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Rocky.get_pm_settings()
    await Rocky.allow_user(chat_id)
