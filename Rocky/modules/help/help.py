...
from Rocky import app, CMD_HELP
from Rocky.helper.PyroHelpers import ReplyCheck
from Rocky.helper.utility import split_list
...
async def module_help(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        try:
            ...
            ac = PrettyTable()
            ac.header = False
            ac.title = "Rocky-UserBot Plugins"
            ac.align = "l"
            for x in split_list(sorted(CMD_HELP.keys()), 2):
                ac.add_row([x[0], x[1] if len(x) >= 2 else None])
            xx = await client.send_message(
                message.chat.id,
                f"```{str(ac)}```\n• @SOFIA_VC_BOTS × @SOFIA_TG_BOTS •",
